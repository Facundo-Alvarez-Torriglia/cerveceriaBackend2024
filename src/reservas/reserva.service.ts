...
export class ReservaService {
    ...
    async eliminarReserva(id:number): Promise <Boolean> {
        try {
            const reservaEliminar: Reserva = await this.getReservaById(id);
            if (reservaEliminar) {
               await this.reservaRepository.remove(reservaEliminar);
               return true; 
            }
        } catch (error) {
            throw new HttpException({ status: HttpStatus.NOT_FOUND,
                error: `Error al intentar eliminar la reserva con id ${id} en la base de datos; ${error}`},
                HttpStatus.NOT_FOUND);
        }
    }
}
